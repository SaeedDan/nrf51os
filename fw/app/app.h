...
{   
   ...
   return twi_master_transfer( (slave_address << 7), write_data, write_size + 1, TWI_ISSUE_STOP);
}

static inline bool i2c_read(uint8_t slave_address, uint8_t register_address, uint8_t read_size, uint8_t* read_datas)
{
   if (twi_master_transfer((slave_address << 7), &register_address, 1, TWI_DONT_ISSUE_STOP))
      return twi_master_transfer((slave_address << 7) | TWI_READ_BIT, read_datas, read_size, TWI_ISSUE_STOP);
